[CmdletBinding()]
param()

$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$rootCmake = Get-Content -Raw (Join-Path $repoRoot "CMakeLists.txt")
$releaseWorkflow = Get-Content -Raw (Join-Path $repoRoot ".github/workflows/artifacts.yml")
$androidCmake = Get-Content -Raw (Join-Path $repoRoot "android/CMakeLists.txt")

if ($rootCmake -notmatch 'target_compile_definitions\s*\(\s*yaml[\s\S]*_POSIX_C_SOURCE=200809L') {
    throw "The Linux host build must expose POSIX strdup() while compiling vendored libyaml."
}

if ($rootCmake -notmatch 'target_compile_definitions\s*\(\s*zip[\s\S]*_GNU_SOURCE') {
    throw "The Linux host build must expose GNU/POSIX types while compiling vendored libzip."
}

if ($releaseWorkflow -notmatch 'gh release upload[\s\S]*--repo "\$GH_REPO"') {
    throw "Release artifact upload must pass an explicit GitHub repository to gh."
}

foreach ($module in @("app", "events", "intent", "ui")) {
    if ($androidCmake -notmatch "src/modules/$module`_module\.c") {
        throw "Android CMake does not compile the built-in Lua module: $module."
    }
}

Write-Host "Host CMake flag regression test passed."
